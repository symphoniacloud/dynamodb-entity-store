...
describe('query', () => {
  ...
  test('query table single entity where multiple returned from table', async () => {
    ...
    const queryResult = await store.for(DOG_ENTITY).query().byPk({ farm: 'Sunflower Farm' })
    expect(queryResult.items.length).toEqual(1)
    expect(queryResult.items[0]).toEqual(chesterDog)
  ...
  test('query GSI single entity where multiple returned from table', async () => {
    ...
    const queryResult = await store.for(DUCK_ENTITY).queryWithGsi().byPk({ coop: 'bristol' })
    expect(queryResult.items.length).toEqual(1)
    expect(queryResult.items[0]).toEqual(waddles)
    expect(queryResult.unparsedItems?.length).toEqual(1)
    expect(queryResult.unparsedItems?.[0]).toEqual({
...
describe('scan', () => {
  ...
  test('scan single entity where multiple returned from table', async () => {
    ...
    const scanResult = await store.for(SHEEP_ENTITY).scan()
